<template>
  <div class="playground-container">
    <header class="header">
      <h1>Status Indicator Playground</h1>
...
const demoStatus = ref('running');
const demoLanguage = ref('');
const activeTab = ref('vue3');
const isCopied = ref(false);

const generatedCode = computed(() => {
  const langAttr = demoLanguage.value ? ` language="${demoLanguage.value}"` : '';
  const componentTag = `<StatusIndicator status="${demoStatus.value}"${langAttr} />`;

  if (activeTab.value === 'vue3') {
    return `<script setup>
import StatusIndicator from 'status-component-vue3';
import 'status-component-vue3/style.css';
<\/script>
...
const highlightedCode = computed(() => {
  return hljs.highlight(generatedCode.value, { language: 'xml' }).value;
});
